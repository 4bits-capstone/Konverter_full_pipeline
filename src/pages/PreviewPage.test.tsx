...
describe('Preview and report pages', () => {
  ...
  it('publishes an approved document to WordPress and offers only the draft preview link', async () => {
    const publish = vi.spyOn(publicationService, 'publishToWordPress')
    renderPreview()
    const button = await screen.findByRole('button', { name: 'Publish to WordPress' })
    await waitFor(() => expect(button).toBeEnabled())
    fireEvent.click(button)

    const view = await screen.findByRole('link', { name: 'View on WordPress' })
    expect(publish).toHaveBeenCalledWith('test-document')
  ...
  it('shows a safe WordPress error and allows retry', async () => {
    vi.spyOn(publicationService, 'publishToWordPress')
      .mockRejectedValueOnce(new Error('WordPress publishing is not configured on the server.'))
    renderPreview()
    const button = await screen.findByRole('button', { name: 'Publish to WordPress' })
    await waitFor(() => expect(button).toBeEnabled())
    fireEvent.click(button)
    ...
    expect(await screen.findByRole('link', { name: 'View on WordPress' })).toBeInTheDocument()
  })

  it('waits for the saved WordPress status before allowing a new draft', async () => {
    let resolveStatus!: (value: null) => void
    vi.spyOn(publicationService, 'getWordPressPublication')
    ...
    resolveStatus(null)
    await waitFor(() => expect(button).toBeEnabled())
    expect(screen.getByText('Save a draft to WordPress')).toBeInTheDocument()
  })

  it('blocks creation when the saved status cannot be checked and offers a read-only retry', async () => {
    vi.spyOn(publicationService, 'getWordPressPublication')
      .mockRejectedValueOnce(new Error('Unavailable'))
    const publish = vi.spyOn(publicationService, 'publishToWordPress')
    renderPreview()
    expect(await screen.findByRole('alert')).toHaveTextContent('Could not check for an existing WordPress draft')
    const button = screen.getByRole('button', { name: 'Publish to WordPress' })
    expect(button).toBeDisabled()
  ...
  it('shows an existing draft without publishing again', async () => {
    const saved = await publicationService.publishToWordPress('test-document')
    vi.spyOn(publicationService, 'getWordPressPublication').mockResolvedValue(saved)
    const publish = vi.spyOn(publicationService, 'publishToWordPress')
    renderPreview()
    expect(await screen.findByRole('link', { name: 'View on WordPress' })).toHaveAttribute('href', saved.previewUrl)
    expect(publish).not.toHaveBeenCalled()
    expect(screen.queryByRole('button', { name: 'Publish to WordPress' })).not.toBeInTheDocument()
    expect(document.querySelector('a[href*="wp-admin"]')).toBeNull()
  })

  it('does not automatically retry ambiguous WordPress failures', async () => {
    const publish = vi.spyOn(publicationService, 'publishToWordPress')
      .mockRejectedValue(new Error('Check WordPress Pages before retrying; a draft may already exist.'))
    renderPreview()
    const button = await screen.findByRole('button', { name: 'Publish to WordPress' })
    await waitFor(() => expect(button).toBeEnabled())
    fireEvent.click(button)
    expect(await screen.findByRole('alert')).toHaveTextContent('Check WordPress Pages before retrying')
