...
function JobStatus({ job }: { job: DocumentProcessingJob }) {
  ...
  if (job.state === "failed")
    return <span className="job-status failed">Needs retry</span>;
...
export function UploadPage() {
  ...
  const acceptFiles = (files: FileList | File[]) => {
    const selectedPdfs = Array.from(files).filter(
      (file) =>
    ...
    if (remainingSlots <= 0) {
      showToast(
        `Remove a document before adding another. The limit is ${MAX_DOCUMENTS}.`,
      );
      return;
    }
    const pdfs = selectedPdfs.slice(0, remainingSlots);
    const limitNote =
      selectedPdfs.length > remainingSlots
        ? ` Only ${remainingSlots} were added because the limit is ${MAX_DOCUMENTS}.`
        : "";
    showToast(
      `Uploading ${pdfs.length} document${pdfs.length === 1 ? "" : "s"}…`,
    );
    void documentService
      ...
      .then((uploadedDocuments) => {
        ...
        showToast(
          `${uploadedDocuments.length} document${uploadedDocuments.length === 1 ? "" : "s"} added to the processing queue.${limitNote}`,
        );
      })
      .catch((error: unknown) =>
        showToast(
  ...
  const selectedJob = activeDocument
    ? documentProcessing[activeDocument.id]
    : undefined;
  ...
  return (
    <section className="screen active" aria-labelledby="upload-heading">
      <div className="upload-wrap upload-wrap-wide">
        ...
        <h2 id="upload-heading" style={{ fontSize: 26, marginTop: 8 }}>
          Upload documents
        </h2>
        <p className="lead">
        ...
        {!documents.length ? (
          ...
          >
              ...
              <div>
                ...
                <div className="queue-counts" aria-live="polite">
                  ...
                  <span className="ready-count">
                    {completedDocuments.length} ready
                  </span>
                </div>
              </div>
              ...
              <div className="banner banner-ok ready-documents-banner">
                ...
                <div>
                  <b>
                    ...
                    {completedDocuments.length === 1 ? " is" : "s are"} ready to
                    review.
                  </b>
                ...
                <button
                  ...
                  onClick={() =>
                    activeDocument && reviewDocument(activeDocument.id)
                  }
                >
            ...
            <div
              className="concurrent-document-list"
              role="radiogroup"
              aria-label="Choose document to review"
            >
              {documents.map((document) => {
                ...
                return (
                  <article
                    className={`concurrent-document-card ${activeDocumentId === document.id ? "selected" : ""}`}
                    key={document.id}
                  >
                    <label className="concurrent-document-main">
                      ...
                      />
                      <span className="file-ic" aria-hidden="true">
                        PDF
                      </span>
                      <span className="file-meta">
                        ...
                        <span className="file-sub">
                          <span className="mono">
                            {document.sizeLabel ?? "4.7 MB"}
                          </span>{" "}
                          ·{" "}
                          {document.pages ? (
                            <span className="mono">{document.pages} pages</span>
                          ) : (
                    ...
                    <div className="concurrent-job-state">
                      <div className="job-state-line">
                        ...
                        {job.state === "running" && (
                          <span className="job-time">
                            <Clock3 />
                            About {formatRemainingTime(job.remainingSeconds)}
                          </span>
                        )}
                      </div>
                      <div
                        className="job-progress"
                        aria-label={`${document.fileName} ${job.progress}% processed`}
                      >
                      ...
                      <span className="job-step">
                        {job.state === "running"
                          ? (job.message ??
                            `${step?.label ?? "Processing"}${step?.detail ? ` — ${step.detail}` : ""}`)
                          : job.state === "failed"
                            ? (job.message ??
                              "Some content could not be extracted from this document. Try uploading another copy of the PDF.")
                            : job.state === "complete"
                              ? "Processing complete"
                              : "Ready to start"}
                      </span>
                    ...
                    <div className="concurrent-document-actions">
                      ...
                      <button
                        className="btn btn-ghost btn-sm document-remove"
                        type="button"
            ...
            <div className="processing-queue-footer">
              <button className="btn btn-outline" onClick={chooseFiles}>
                <RefreshCcw />
                Re-upload or add more documents
              </button>
            </div>
          </div>
        )}
