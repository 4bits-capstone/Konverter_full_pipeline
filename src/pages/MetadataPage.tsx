...
import { Check, CircleCheck, ExternalLink, Info, LoaderCircle, Pencil, Plus, RotateCcw, Save, Trash2, TriangleAlert } from 'lucide-react'
...
import { approvalService, metadataService, publicationService } from '../services'
...
export function MetadataPage() {
  ...
  const [pendingApprovalMetadata, setPendingApprovalMetadata] = useState<DocumentMetadata | null>(null)
  ...
  const approvalChecks = [
    {
      ...
      detail: approvalMetadata.title.trim() || activeDocument?.title || 'No document title available',
    },
    {
      label: 'Review flags resolved',
      ok: pendingCount === 0,
      detail: pendingCount ? `${pendingCount} of ${reviewItems.length} flags still need a decision` : `All ${reviewItems.length} flags have a decision`,
    },
    {
      label: 'Metadata confirmed',
      ok: requiredMetadataComplete,
      detail: requiredMetadataComplete ? `${approvalMetadata.publisher} · ${approvalMetadata.publishedDate}` : 'Title, publisher or published date is missing',
    },
    {
      label: 'Accessible output ready',
      ok: pendingCount === 0 && requiredMetadataComplete,
      detail: 'Accessible HTML, landing page, structured JSON and embedded JSON-LD will be generated',
    },
  ]
  ...
  const approveAndOpenPreview = async () => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['publication', activeDocumentId] })
      unlock('preview')
      markDone('approval')
      ...
      showToast(error instanceof Error ? error.message : 'Approval failed. Try again.')
    } finally {
      setApproving(false)
    }
  }
  ...
  const renderField = (config: FieldConfig) => {
    ...
    return (
      <div key={name} className={`metadata-field is-${tone} ${editing ? 'editing' : ''}`} role="listitem" onFocus={() => setActiveEvidence(name)}>
    ...
    <section className="screen active" aria-labelledby="metadata-heading">
      <div className="section-title metadata-titlebar">
        <div>
          ...
          <p className="lead">Confirm the source details people will use to identify and cite this report.</p>
        </div>
        ...
        <div className="meta-grid">
          <form className="panel" onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <div className="panel-pad metadata-fields">
              <div className="metadata-field-list" role="list" aria-label="Metadata review fields">
                {fields.map(renderField)}
              </div>
              <div className="metadata-form-actions">
                <button className="btn btn-primary" type="submit" disabled={isSubmitting || reviewCount > 0}>{isSubmitting ? 'Saving…' : reviewCount > 0 ? `Resolve ${reviewCount} field${reviewCount === 1 ? '' : 's'} to continue` : 'Run final system checks →'}</button>
                <button className="btn btn-outline" type="button" onClick={() => navigate(converterStagePath('review'))}>Return to review</button>
              </div>
            </div>
          </form>
      ...
      {approvalModalOpen ? (
        <div className="overlay show" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !approving) setApprovalModalOpen(false) }}>
          <section className="modal approval-popup" role="dialog" aria-modal="true" aria-labelledby="confirm-approval-heading">
            <div className="modal-ic"><CircleCheck aria-hidden="true" /></div>
            <span className="eyebrow">Final system checks</span>
            <h3 id="confirm-approval-heading">Approve {approvalMetadata.title || 'this document'}?</h3>
            <p>Konverter checked the document state automatically. No separate reviewer checklist or approval page is required.</p>
            <ul className="checklist" aria-label="Approval system checks">
              {approvalChecks.map((check) => (
                <li className={`check ${check.ok ? 'ok' : 'blocked'}`} key={check.label}>
                  <span className="checkbox" role="img" aria-label={check.ok ? 'Passed' : 'Blocked'}><Check aria-hidden="true" /></span>
                  <div><div className="check-txt">{check.label}</div><div className="check-sub">{check.detail}</div></div>
                </li>
              ))}
            </ul>
            <div className="approval-popup-summary" aria-label="Document approval summary">
              <span><b>File</b>{activeDocument?.fileName || 'Not available'}</span>
              <span><b>Review flags</b>{reviewItems.length - pendingCount}/{reviewItems.length} resolved</span>
              <span><b>Metadata</b>{requiredMetadataComplete ? 'Confirmed' : 'Incomplete'}</span>
            </div>
            <div className="modal-actions">
              <button className="btn btn-outline" type="button" disabled={approving} onClick={() => setApprovalModalOpen(false)}>Cancel</button>
              <button ref={approvalButtonRef} className="btn btn-seal" type="button" disabled={!approvalReady || approving} onClick={approveAndOpenPreview}>{approving ? 'Generating outputs…' : 'Approve and open preview'}</button>
            </div>
          </section>
        </div>
      ) : null}
