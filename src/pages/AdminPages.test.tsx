...
describe('Admin console preserves the original workflow', () => {
  ...
  it('keeps document details collapsed until requested and expands them within the table', async () => {
    renderAdmin('/admin/documents')
    await screen.findByRole('table', { name: 'Document list' })
    expect(screen.queryByRole('complementary', { name: 'Selected document details' })).not.toBeInTheDocument()
    const trigger = screen.getByRole('button', { name: `Show document details: ${testDocument.title}` })
    fireEvent.click(trigger.closest('tr')!.querySelector('td:nth-child(2)')!)
    const details = screen.getByRole('complementary', { name: 'Selected document details' })
    expect(details.closest('tr')).toHaveClass('audit-inline-detail-row')
    expect(within(details).getByRole('heading', { name: testDocument.title })).toBeInTheDocument()
    expect(trigger).toHaveAttribute('aria-expanded', 'true')
    fireEvent.click(within(details).getByRole('button', { name: 'Close document details' }))
    expect(screen.queryByRole('complementary', { name: 'Selected document details' })).not.toBeInTheDocument()
  })
  ...
  it('opens approved documents in Preview and disables opening incomplete documents', async () => {
    renderAdmin('/admin/documents')
    await screen.findByRole('table', { name: 'Document list' })
    const failedRow = screen.getByRole('button', { name: 'Show document details: Failed report' }).closest('tr')!
    expect(within(failedRow).getByRole('button', { name: 'Open' })).toBeDisabled()
    fireEvent.click(screen.getByRole('button', { name: 'Show document details: Approved report' }))
    const details = screen.getByRole('complementary', { name: 'Selected document details' })
  ...
  it('retains audit selection, before/after evidence and chain status across filters', async () => {
    ...
    expect(screen.queryByRole('complementary', { name: 'Selected action details' })).not.toBeInTheDocument()
    const trigger = within(table).getByRole('button', { name: /Show action details/i })
    fireEvent.click(trigger.closest('tr')!.querySelector('.audit-summary-cell')!)
    const details = screen.getByRole('complementary', { name: 'Selected action details' })
    expect(details.closest('tr')).toHaveClass('audit-inline-detail-row')
  ...
  it('retains pagination and collapses details when the expanded entry leaves the page', async () => {
    ...
    fireEvent.click(screen.getAllByRole('button', { name: /Show action details/i })[0])
